...
def train_model(model,
    ...
    model_path = "./model_in_traininghighdroout.pt"
    ...
    for epoch in range(1, num_epochs + 1):
        lambda_title = 0.02 if epoch > 0 else 0
        p_Lef_drop = 0.9 - 0.7 * (epoch-0)/(num_epochs-0 + 1e-9)        
        ...
        if epoch % validate_every == 0 or epoch == num_epochs:
            ...
            fig_ax = plot_metrics_live(
                ...
                "TrueTransformer-newedgeshighdroout",
                xlabel="Epoch",
                ylabel_left="Loss",
                ylabel_right="P · R · F1",
...
# %%
#Save model
torch.save(model.state_dict(), "TrueTransformer-neweedgeshighdroout.pt")
